...
# Les 20 premiers titres (pour garder la heatmap lisible)
N = 20  # ou n'importe quelle taille adaptée
titles_subset = data['title'].iloc[:N]
indices_subset = [indices[title] for title in titles_subset]
cosine_sim_subset = cosine_sim[indices_subset, :][:, indices_subset]
...
plt.title('Heatmap de similarité cosinus (TF-IDF genres) pour un échantillon de titres')
...
# Ce qui est proche de 1 est analogue, et de 0 est différent.

# In[60]:


# Sous-ensemble de 30 titres (pour la lisibilité)
N = 30  # ou adapte cette taille !
titles_subset = data['title'].iloc[:N].tolist()
indices_subset = [indices[title] for title in titles_subset]

# Création du graphe
G = nx.Graph()
for i, titre_i in enumerate(titles_subset):
    for j, titre_j in enumerate(titles_subset):
        if i < j:  # éviter les doublons, graphe non orienté
            sim = cosine_sim[indices_subset[i], indices_subset[j]]
            if sim > 0.4:  # SEUIL, ajuste selon ce qui te paraît pertinent après test
                G.add_edge(titre_i, titre_j, weight=sim)
...
nx.draw(
    G, pos,
    with_labels=True, node_size=700, font_size=9,
    edge_color='orange', node_color='skyblue'
)
plt.title("Graphe de similarité cosinus (TF-IDF genres) des titres")
plt.tight_layout()
plt.show()


# * Les nœuds proches ou connectés représentent des contenus qui partagent des genres proches, donc susceptibles d'intéresser un même public.
# * Les clusters dans le graphe correspondent à des groupes thématiques (e.g. films d'horreur, comédies, séries policières) peuvent former des sous-graphes denses avec beaucoup de connexions internes.
# * Les contenus isolés ou avec peu de liens ont des genres plus spécifiques ou atypiques, donc une audience plus ciblée.
# * Cette représentation permet d’affiner la recommandation de contenu en proposant des films ou séries voisins dans le graphe, donc très similaires en termes de genres et styles.
# * Le graphe illustre visuellement l’approche dite de recommandation par similarité de contenu qui est très efficace quand on connaît peu l’utilisateur, reposant sur des caractéristiques objectives des films/séries.
# 
# Finalement, ce type de graphe est un puissant outil pour comprendre la structure interne d’un catalogue de contenu et déceler des groupes 
# thématiques naturels qui aident à orienter les recommandations personnalisées sur Netflix ou autres plateformes.

# ### _La projection t-SNE (t-distributed Stochastic Neighbor Embedding)_
# Est une méthode de réduction de dimension très utilisée pour visualiser des données en haute dimension (e.g. des vecteurs TF-IDF issus des genres de films) dans un espace à deux ou trois dimensions.  
# Un nuage de points où chaque point représente un contenu, et où les points proches dans cet espace réduit sont des contenus similaires dans l’espace d’origine.

# Ça
# * Simplifie la visualisation d’un espace très complexe, en passant de centaines ou milliers de dimensions (chaque genre est une dimension TF-IDF) à 2D ou 3D.
# * Met en exergue des clusters : regroupements de films/séries similaires (par exemple, films d’horreur, comédies, drames) apparaissent naturellement proches dans le graphe.
# * Aide à déceler des anomalies ou d’outliers : contenus très spécifiques ou atypiques apparaissent isolés.
# * Aide à comprendre la structure interne des données pour affiner des recommandations, guider le choix de clusters, ou explorer visuellement des similarités complexes.
# * Utile dans la phase exploratoire avant de construire des modèles plus avancés (clustering, classification, etc.).

# In[61]:


# Conversion en array numpy dense
plt.figure(figsize=(12,10))

# Convertir la matrice TF-IDF sparse en array dense numpy
tfidf_dense = tfidf_matrix.toarray()

# Créer le modèle t-SNE
tsne = TSNE(n_components=2, metric='cosine', random_state=42)

# Calculer la projection t-SNE (c'est ce qui crée 'embedding')
embedding = tsne.fit_transform(tfidf_dense)

# Indices des films
idx_movies = np.where(data['type'] == 'Movie')[0]
plt.scatter(embedding[idx_movies, 0], embedding[idx_movies, 1], 
            c='blue', label='Films', alpha=0.6)

# Indices des séries
idx_shows = np.where(data['type'] == 'TV Show')[0]
plt.scatter(embedding[idx_shows, 0], embedding[idx_shows, 1], 
            c='orange', label='Séries', alpha=0.6)

plt.title('Projection t-SNE des contenus Netflix')
...
# Il y a plus de films que de séries.  
# Il est logique qu'il y ait plus de cluster pour les films et que certains soient plus gros.  
# Il semble y avoir beaucoup de moyens clusters.
# 
# #### ⚠️ Dans ce genre de graphiques, les dimensions sont latentes, et sont des coordonnées abstraites.
